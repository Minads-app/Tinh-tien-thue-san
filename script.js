// --- DATA INITIALIZATION ---
// Đơn giá được cập nhật theo giờ (Ví dụ)
const DEFAULT_RULES = [
    { id: 1, group: 'Cầu lông', name: 'Sân Cầu lông (Sáng/Chiều T2-T6)', days: [1,2,3,4,5], start: '06:00', end: '17:30', price: 50000 },
    { id: 2, group: 'Cầu lông', name: 'Sân Cầu lông (Tối T2-T6)', days: [1,2,3,4,5], start: '17:30', end: '22:00', price: 70000 },
    { id: 3, group: 'Cầu lông', name: 'Sân Cầu lông (Cuối tuần)', days: [6,0], start: '06:00', end: '22:00', price: 70000 },
    { id: 4, group: 'Bóng đá', name: 'Sân Bóng đá (Sáng)', days: [0,1,2,3,4,5,6], start: '06:00', end: '17:00', price: 300000 },
    { id: 5, group: 'Bóng đá', name: 'Sân Bóng đá (Tối)', days: [0,1,2,3,4,5,6], start: '17:00', end: '22:00', price: 400000 },
];
...
function formatVND(amount) {
    // Format: 200.000 ₫ (Dùng replace để thay dấu phẩy mặc định của en-US thành chấm nếu muốn hardcode, hoặc dùng locale vi-VN)
...
function backupData() {
    ...
    downloadAnchorNode.setAttribute("download", "bang_gia_san_" + new Date().toISOString().slice(0,10) + ".json");
...
function renderConfigTable() {
    ...
    pricingRules.forEach(rule => {
        const daysText = rule.days.length === 7 ? 'Tất cả các ngày' : 
                         rule.days.map(d => d === 0 ? 'CN' : 'T'+(d+1)).join(', ');
        ...
        tr.innerHTML = `
            ...
            <td class="p-3 border text-xs text-gray-500 max-w-xs break-words">${daysText}</td>
...
function saveRule() {
    ...
    if(!name || days.length === 0) {
        Swal.fire('Lỗi', 'Vui lòng nhập tên và chọn ít nhất 1 ngày!', 'error');
        return;
    }
...
function addToBill() {
    ...
    if(duration <= 0) {
        Swal.fire('Lỗi', 'Giờ kết thúc phải lớn hơn giờ bắt đầu', 'error'); return;
    }
    ...
    if(isNaN(startDate) || isNaN(endDate) || endDate < startDate) {
        Swal.fire('Lỗi', 'Ngày không hợp lệ', 'error'); return;
    }
    if(selectedDays.length === 0) {
        Swal.fire('Lỗi', 'Vui lòng chọn thứ trong tuần', 'error'); return;
    }
    ...
    if(count === 0) {
        Swal.fire('Thông báo', 'Không có ngày tính tiền (do bộ lọc hoặc ngày nghỉ)', 'warning'); return;
    }

    // Add Item (NEW LOGIC: COUNT * DURATION * PRICE)
